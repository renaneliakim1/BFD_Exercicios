...
if (isNaN(valor)) {
  console.log("Erro: valor inválido. Digite um número válido.");
  process.exit();
}

let escala = prompt(
  "Digite a escala original (C para Celsius ou F para Fahrenheit):"
).toUpperCase();

if (escala !== "C" && escala !== "F") {
  console.log(
    "Erro: escala inválida. Use 'C' para Celsius ou 'F' para Fahrenheit."
  );
  process.exit();
}
